...
export function ListingCard({
  ...
  return (
    <Link href={`/listing/${listing.id}`}>
      <div
        className={`group cursor-pointer relative card-hover ${featured ? "md:col-span-2 md:row-span-2" : ""}`}
      >
        <div
          className="relative aspect-[3/4] md:aspect-[4/5] overflow-hidden w-full h-full rounded-sm"
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 md:p-5">
            <div className="space-y-2">
              ...
              <h3
                className={`font-display font-bold leading-tight text-white ${featured ? "text-2xl md:text-3xl" : "text-lg md:text-xl"}`}
              >
                {listing.title}
              </h3>
...
export function ListingCardSkeleton() {
  return (
    <div
      className="relative aspect-[3/4] md:aspect-[4/5] overflow-hidden rounded-sm animate-pulse"
      style={{ backgroundColor: "var(--color-secondary)" }}
    >
      <div
        className="absolute top-3 left-3 w-16 h-6 rounded"
